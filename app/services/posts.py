...
async def is_subcategory_in_category(
    db: AsyncSession, category_id: int, sub_category_id: int
):
...
async def perfome_create_subcategory(
    ...
    category = await crud_category.get(
        db, title=await clean_title(subcategory_data.category)
    )
    if not category:
        raise HTTPException(
    ...
    subcategory_data_dict[
        "category_id"
    ] = category.id  # use the retrieved category's ID
    ...
    subcategory_info_data[
        "category"
    ] = subcategory_data.category  # use the retrieved category's title
