...
async def calculate_percentage_change(old_value, new_value):
    ...
    if new_value == 0 and old_value == 0:
        return "same"  # If both values are zero, the percentage change is 0%
    elif new_value == 0:
        return "-100%"  # If the new value is zero, it's a 100% decrease
    elif old_value == 0:
        return f"+{new_value}"  # If the old value is zero, it's an infinite increase percentage-wise
