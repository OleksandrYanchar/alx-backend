...
from schemas.users import UserCreateInSchema, UserCreateOutSchema, UserPasswordChangeSchema, UserPasswordResetSchema
...
async def create_user(
    user_request: UserCreateInSchema, request: Request, db: AsyncSession = Depends(get_async_session)
) -> UserCreateOutSchema:
    """
    ...
    if await validate_password(user_request.password) and await validate_email(user_request.email):
        if await is_unique(db, Users, {"email": user_request.email, "username": user_request.username}):
            try:
                # Hash password and create user
                user_request.password = get_password_hash(user_request.password)
                obj_in = UserCreateInSchema(**user_request.dict())
                new_user = await crud_user.create(db, obj_in)  # This should return the new user object with an ID
                
                await verify_email_sender.send_email(new_user, request)
                
                # Generate tokens using the new_user object, which has an 'id'
                tokens = await create_jwt_tokens(new_user)  # Pass new_user instead of user_request
...
async def change_password(user_request: UserPasswordChangeSchema, db: AsyncSession = Depends(get_async_session), user: Users = Depends(get_current_user)):
...
async def refresh_token(
    ...
    try:
        ...
        tokens = await create_jwt_tokens(user)  # Pass the user object to create new tokens
    ...
    if not await is_token_blacklisted(token, db):  # Fix #1: await the function
        ...
        try:
            ...
            if user:
                ...
                token_schema = await create_jwt_tokens(user)  # Await the async function call
