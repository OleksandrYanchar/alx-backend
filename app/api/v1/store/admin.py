...
async def generate_report(db: AsyncSession = Depends(get_async_session)) -> DailyReportScheme:
    ...
    try:
        ...
        users_before_yesterday = await crud_user.get_total_before(db, 'joined_at', yesterday)
        ...
        vips_before_yesterday = await crud_user.get_total_before(db, 'viped_at', yesterday)
        ...
        posts_before_yesterday = await crud_post.get_total_before(db, 'created_at', yesterday)

        new_users = total_users - users_before_today
        new_posts = total_posts - posts_before_today 
        new_vips = total_vips - vips_before_today

        new_users_yestarday = total_users - users_before_yesterday
        new_posts_yestarday = total_posts - posts_before_yesterday
        new_vips_yestarday = total_vips - vips_before_yesterday

        activated, total_activated = await crud_user.get_multi_filtered(db, is_activated=True)


        users_change = await calculate_percentage_change(new_users, new_users_yestarday)
        posts_change = await calculate_percentage_change(new_posts, new_posts_yestarday)
        vips_change = await calculate_percentage_change(new_vips, new_vips_yestarday)
